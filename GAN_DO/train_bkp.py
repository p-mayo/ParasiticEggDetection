...
def save_image(img, prediction, target, filename):
    ...
    img = draw_boxes(unnorm(img).permute(1,2,0).numpy().copy(), 
        new_outputs['boxes'], new_outputs['labels'])
...
def main():
    ...
    dataset = ped.ParasiticEggDataset(paths, get_targets(targets, range(len(paths))),
        ...
        with torch.no_grad():
            for i in np.random.randint(0,len(dataset), 10):
                ...
                with torch.no_grad():
                    ...
                    filename = '%s_pred_hq.png' % paths[i].split(os.path.sep)[-1].split('.')[0]
                    save_image(x_real[0].to('cpu'), y_real, y, filename)
                    filename = '%s_pred_lq.png' % paths[i].split(os.path.sep)[-1].split('.')[0]
